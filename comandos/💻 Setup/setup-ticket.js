            ...
            if(channel) {
                ...
                await quemensaje.channel.awaitMessages({
                    ...
                    const msg = await message.guild.channels.cache.get(objeto.canal).send({
                        embeds: [new Discord.MessageEmbed()
                            .setAuthor(`Bienvenid@ al panel de ayuda` , client.user.displayAvatarURL())
                            .setDescription(`${message.content.substring(0, 2048)}`)
                            .setImage('https://cdn.discordapp.com/attachments/1002258235392282696/1002267302831603773/TicketFalcon.png')
                            .setTimestamp()
                            .setColor(client.color)
                        ],
                        components: [new Discord.MessageActionRow().addComponents(new Discord.MessageButton().setLabel("Crear un Ticket").setCustomId("crear_ticket").setStyle("SUCCESS"))]
                    })
